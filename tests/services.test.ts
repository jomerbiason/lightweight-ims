...
describe('customer credit ledger',()=>{
 ...
 it('allows paying off a balance exactly despite floating-point drift',()=>{const d=makeWithCustomer();chargeCustomer(d,'c',10.10,'Sale');chargeCustomer(d,'c',20.20,'Sale');expect(d.customers[0].balance).not.toBe(30.30);expect(()=>recordPayment(d,'c',30.30)).not.toThrow();expect(d.customers[0].balance).toBeCloseTo(0);});
