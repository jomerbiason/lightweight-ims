...
async function sessions(){
  ...
  if(open){
    ...
    const expected=(open.openingCash||0)+data!.transactions.filter(tx=>tx.type==='SALE'&&tx.timestamp>=open.openedAt&&!(tx.referenceId&&chargedRefs.has(tx.referenceId))).reduce((s,tx)=>s+(-tx.quantityChange)*(tx.salePrice??0),0);
