...
function sales(){
  ...
  ${data!.customers.filter(c=>!c.archived).length?`<label>${t(data!,'chargeTo')}<select id="cart-customer"><option value="">${t(data!,'cash')}</option>${data!.customers.filter(c=>!c.archived).map(c=>`<option value="${c.id}">${esc(c.name)} (${money(c.balance)})</option>`).join('')}</select></label>`:''}
...
async function sessions(){
  ...
  if(open){
    const expected=(open.openingCash||0)+data!.transactions.filter(tx=>tx.type==='SALE'&&tx.timestamp>=open.openedAt).reduce((s,tx)=>s+(-tx.quantityChange)*(tx.salePrice??0),0);
...
function render(){if(!data)return;if(locked)return lockScreen();let content=page==='home'?home():page==='sales'?sales():page==='products'?products():page==='low'?low():page==='buy'?buy():page==='customers'?customers():page==='history'?history():page==='reports'?reports():backup();if(page==='home')content+=`<section class="section card"><h2>Store tools</h2><div class="actions"><button class="secondary" data-page="reports">Reports</button><button class="secondary" id="categories">Categories</button><button class="secondary" id="suppliers">${t(data,'suppliers')}</button><button class="secondary" id="session">${data.sessions.some(s=>!s.closedAt)?'Close Store':'Open Store'}</button></div></section>`;layout(content);bind()}
