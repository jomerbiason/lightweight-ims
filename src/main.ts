...
document.documentElement.dataset.theme='dark'; // system-wide default; applyAppearance() overrides once a store's own setting loads
...
function lockScreen(){
  ...
  $('#unlock-form')!.addEventListener('submit', async e=>{
    ...
    if(hash===data!.settings.pin){locked=false;if(licenseState()&&data!.settings.syncRole==='display'&&data!.settings.syncCode)startSyncPolling(String(data!.settings.syncCode));render()}
...
function appearanceModal(){
  ...
  m.querySelector('#reset-appearance')?.addEventListener('click',async()=>{
    data!.settings.theme='dark';data!.settings.accent='#166534';applyAppearance();await persist();m.remove();toast(t(data!,'settingsSaved'));render();
  });
}
