import './styles.css';
import { id, now, status, expirationStatus, estimatedProfit, type StoreData, type Product, type Category, type InventoryTransaction } from './domain';
...
function appearanceModal(){
  ...
  const m=modal(t(data!,'settings'),`<div class="settings-grid">
    ...
    <div class="setting-card"><label>${t(data!,'pinLock')}
    ...
    <div class="setting-card"><label>${t(data!,'notifications')}
...
function frequentlySold(limit=6){
  ...
  for(const tx of data!.transactions)if(tx.type==='SALE'&&new Date(tx.timestamp).getTime()>=monthAgo)soldQty.set(tx.productId,(soldQty.get(tx.productId)||0)+(-tx.quantityChange));
...
function sales(){
  ...
  ${cart.length?`<section class="section card"><h2>${t(data!,'cart')}</h2>${cartRows}<div class="actions" style="justify-content:space-between;align-items:center;margin-top:12px;flex-wrap:wrap"><strong>${t(data!,'total')}: ${money(cartTotal)}</strong><div class="actions"><button class="secondary" id="clear-cart">${t(data!,'clearCart')}</button><button class="primary" id="checkout">${t(data!,'checkout')}</button></div></div></section>`:''}`}
...
function reports(){const active=data!.products.filter(p=>p.active),sales=data!.transactions.filter(t=>t.type==='SALE'),movements=data!.transactions.length,lowCount=active.filter(p=>status(p)==='low').length,out=active.filter(p=>status(p)==='out').length;
...
async function checkout(){
  ...
  try{
    ...
    await persist();
    const receiptText=buildCartReceipt(items,now());
    cart=[];
...
function render(){if(!data)return;if(locked)return lockScreen();let content=page==='home'?home():page==='sales'?sales():page==='products'?products():page==='low'?low():page==='buy'?buy():page==='history'?history():page==='reports'?reports():backup();if(page==='home')content+=`<section class="section card"><h2>Store tools</h2><div class="actions"><button class="secondary" data-page="reports">Reports</button><button class="secondary" id="categories">Categories</button><button class="secondary" id="session">${data.sessions.some(s=>!s.closedAt)?'Close Store':'Open Store'}</button></div></section>`;layout(content);bind()}
